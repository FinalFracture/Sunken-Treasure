...
class TradeMenu(pygame.sprite.Sprite):
        ...
        for item_slot in self.transactable_spaces:
            if item_slot.rect.collidepoint(self.mouse_pos) and pygame.mouse.get_pressed()[0]:
                if item_slot.subject and not self.clicking:
                    item_slot.click()
                    if item_slot.subject in self.owner.inventory and not self.clicking and item_slot not in self.buy_cart:
                        self.buy_cart.append(item_slot)
                        self.clicking = True
                    if item_slot.subject in self.interactor.inventory and not self.clicking and item_slot not in self.sell_cart:
                        self.sell_cart.append(item_slot)
                        self.clicking = True
                    if item_slot in self.sell_cart and not self.clicking:
                        self.sell_cart.remove(item_slot)
                        self.clicking = True
                    if item_slot in self.buy_cart and not self.clicking:
                        self.buy_cart.remove(item_slot)
                        self.clicking = True
      
    def _sell(self) -> None: 
        #transact player inventory out, and into the shop inventory. then update the sprites
        if self.owner.gold >= sum(item.subject.value for item in self.sell_cart):
            for item in self.sell_cart:
                self.owner.gold -= item.subject.value
                self.interactor.gold += item.subject.value
                self.interactor.inventory.remove(item.subject)
                self.owner.inventory.append(item.subject)
                item.subject = None
    ...
    def _buy(self) -> None: 
        ...
        if self.interactor.gold >= sum(item.subject.value for item in self.buy_cart):
            for item in self.buy_cart:
                self.interactor.gold -= item.subject.value
                self.owner.gold += item.subject.value
                self.owner.inventory.remove(item.subject)
                self.interactor.inventory.append(item.subject)
                item.subject = None
            self.owner.inventory_ui.menu_refresh()
            self.interactor.inventory_ui.menu_refresh()
            self.buy_cart = []
    ...
    def _update_text(self) -> None:
        ...
        sell_cart_value = sum(item.subject.value for item in self.sell_cart)
        buy_cart_value = sum(item.subject.value for item in self.buy_cart)
